...
class Arcface(nn.Module):
    ...
    def forward(self, features, targets):
        # get cos(theta)
        long_targets = torch.cat([targets, targets], dim=0)
        cos_theta = F.linear(F.normalize(features), F.normalize(self.weight))
        cos_theta = cos_theta.clamp(-1, 1)  # for numerical stability

        target_logit = cos_theta[torch.arange(0, features.size(0)), long_targets].view(-1, 1)
        ...
        cos_theta.scatter_(1, long_targets.view(-1, 1).long(), final_target_logit)
